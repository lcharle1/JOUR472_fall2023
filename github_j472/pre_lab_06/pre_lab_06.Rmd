...
**Task** Load the four CSV population estimate files. Do they have the same column structure? **Answer:** Yes, all four have 24 observations and 9 variables. Aside from the year, they're all the same.
...
**Task** Run the following code to filter on Frederick County's registration data and look at the results. What stands out to you about the changes in party registration over time? **Answer:** The numbers DEM largely increased compared to REP, and other also had a big jump between 2018 to 2020. Overall this leads to an estimated 30,000 increase in total population.
...
**Task** Run the following code to join Maryland population data and the 2022 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Answer:** There are 24.
...
**Task** Load the Maryland demographic data and examine it. What kind of information does it contain? **Answer**: It shows the population over 18, the different races, one races, other races, and two or more races.
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Answer:** The highest percentage of black 18+ population is of all 18+ population is 60% and in Prince George's County. The highest percentage of democratic is also in Prince George's county at 77%.
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **Answer:** The highest pct of 18+ population is at 19% in Howard County, but the highest percentage of democratics is still 77% in Prince George's County.
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N column? **Answer:** There are a lot of \<dbl\>. 35 \<dbl\> and 5 \<chr\>
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N? **Answer:** The mutation of ZCTA5N converted it from a dbl to chr (from number to character).
