...
**Answer** <https://datawrapper.dwcdn.net/hkucu/1/>
...
#pg_call_pct_month <- pg_call|> group_by(month)|> summarize(total_calls = n())|> mutate(pct = total_calls/1397*100)|> arrange(desc(pct))
view(md_gov_county)
...
```{r}
wes_moore_pct_votes <- md_gov_county |>
  group_by(moore, county, fips_code)|>
  summarize(total_votes = n()) |>
  mutate(pct_votes = total_votes/moore*100) |>
  arrange(desc(pct_votes))

view(wes_moore_pct_votes)
```
...
**Task** Look for "Hide regions without data" under Appearance, and click the slider icon to enable that feature. You should see a map zoomed into Maryland with some counties in various colors. Describe the changes from the previous map **Answer**: It removed the rest of the country except for Maryland, which is the only state we have data for. It eliminates visual confusion.
...
**Task** Click the "Customize tooltips" button so it expands down. Change {{ fips_code }} to {{ county }} and {{ pctmoore }} to {{ FORMAT(pctmoore, "0.0%")}}. Describe the changes from the previous map. **Answer:** It changed the name of the labels that popped up when you hovered the curser over the counties. Before it was the number of votes and the fips_code, but now we know the county and the pct of votes.
...
<https://datawrapper.dwcdn.net/v6Kvb/1/>
