...
**A1.** One major difference between the two bar charts is Baltimore County had more overdoses in the warmer months, August and May were the top two, compared to Prince George's County highest month is December, but second highest is July. Baltimore's lowest is February and second lowest is December, and PG's lowest is January and it's second lowest is February. Another major difference are the months considered second lowest in Baltimore County is considered highest in PG County.

```{r}

pg_call_pct_month <- pg_call|>
  group_by(month)|>
...
pg_call_pct_month |>
  ...
  theme_classic()+
  labs (
    title = "Percentage of Overdose Calls Per Month in Prince George's County",
    x =  "Month",
    y =  "Percentage of Calls", 
    caption = "Prince George County EMS"
  )
...
**A2.** I would say there is a consistent one or two calls per month. Around week 26-30, a spike can be found, this month is probably July. But then there is an immediate dip from December to January.

```{r}
pg_call_pct_week <- pg_call |>
  group_by(week)|>
  summarize(total_calls = n())|>
  mutate(pct = total_calls/1397*100)|>
    arrange(desc(pct))
...
```{r}
#scatter_plot <- ggplot(md_median_income_zip, aes(x = MedianIncome, y = TotalCalls)) + geom_point() + geom_text(aes(label = ZipCode), nudge_x = 500) +  labs(title = "Relationship between Median Income and Overdose 911 Calls",x = "Median Income",y = "Total 911 Calls") +theme_minimal()
```

```{r}
md_median_income_zip <- get_asc(geography = 'zcta', variables='B19013_001', state='MD',year=2019)
