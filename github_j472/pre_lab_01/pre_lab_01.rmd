...
**Answer:** umd_courses is telling R to group by the terms and then summarize the count or number of classes.
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer:** The difference is R now organized the column with the term with the most about of classes to least.
...
**Answer:** Now R arranged the departments in alphabetical order while saying what term and how many classes. Now the focus is in the order of the departments so there are a lot of the same terms showing and various results for the count of classes.
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer:** R is now arranging the total seats from highest to lowest. This focuses the results on that so the number is decscending, while showing the term and count class it's in.
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? **Answer:** R now provides a lot more information about the count_classes and shows the terms of each count_class. The focus is in the total_seats as they are declining from highest to lowest. From that R is also providing the mean and median of each count_classes.
...
**Answer:** The arrangement of total_seats is the focus as it decreases from highest to lowest, similarly to the last codeblock. But now we have departments which adds to the number of rows we have.
...
1.  Why was task 13 and task 14 the practically the same (disregarding the departments column) without the use of min() and max() function? Or were they used but not mentioned?

2.  Where would you put the min() and max() functions for task 14? Do I put them where the hashtags are?

3.  How does it summarise () the data? Can't I list the functions within the group_by() function and it will group it?
