...
baltco_911_overdose_calls <- read_csv('lab_05/data/tabula-Baltimore County; Carey, Samantha log OD.csv', col_names = FALSE) 

  
baltco_911_overdose_calls <- baltco_911_overdose_calls |> rename(date = X1, time=X2, case_number=X3, eventype=X4, location=X5)|>
...
**A1**: It would require more extensive research to find out whether there were no overdoses at all. I would also prefer to arrange in descending the order using date instead of date_count like it is right now.
...
baltco_911_overdose_calls |> 
  ...
  #earthquakes |>group_by(type) |>summarise (type_count = n()) |>arrange(desc(type_count))
```
...
**A2:** According to the finds the new column lists which day of the week it occurred on through 1 being Monday and 7 Sunday.
...
baltco_911_overdose_calls |>group_by(weekday)|>summarize(count = n()) |>mutate(pct_total = count/4112*100)|>arrange(desc(pct_total))
...
**A3:** 4540 Silver Spring Rd has the most with 36 calls. I google mapped the road and noticed it is very residential with a small shopping center further down. So there is a good chance the calls occur inside one or more of these homes. One thing that makes me a little bit less confident in the counts are variants or inaccurate data.
...
**A4**: My best idea is investigating 4540 Silver Spring Rd during the weekends, because the data shows there is a high number of calls coming from there during a specific time. So it would be interesting to do a story on why this is happening more here. Maybe the story could also resolve the problem entirely too.
