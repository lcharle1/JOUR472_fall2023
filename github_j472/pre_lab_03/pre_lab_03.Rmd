...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer:** There are no more parsing errors.
...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer:** "Vendor Name" is a character type. "Amount" is a double precision type. "Date" is a character type.
...
**Task** Run the following codeblock to count the number of payments. Looking at the result, what jumps out at you as potentially problematic? **Answer:** The different names for categories. Loan has it's variant L, and another category is contract. As well as the N/A
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **Answer:** I see the amount going down in ascending order with the smallest amount first. The grantor and grantee are also there with other columns.
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does the function change the headers? Answer below. **Answer:** The headers became lower cased.
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **Answer:** There is one duplicate.
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. **Answer:** There are now 17,740 rows.
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer:** Now there is a new column that keeps the five digit zip codes the same and converts nine digit zipcodes to five digit.
