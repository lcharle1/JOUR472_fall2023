...
**Answer:** I think it's important to note specific filters in case you have to go back or want to add more later. In a way its like bookmarking it.
...
**Answer:** Expenditure_date no longer is labeled as a character, but now as a date. This also changed the m/d/y format to y/m/d.
...
**Answer:** All of them are in January and it maintained the year-day-month format.
...
**Answer**: After naming the code as general_22, you asked the code to mutate the total votes, Cox, Moore, Lashar, Wallace and write_ins. Mutate adding them all together and dividing by Moore to get pct_moore.
...
**Answer:** In the second code, the function arrange() is telling the computer to move the data around and order it in a specific way. Compared to the filter function above.
...
**Answer:** With the arrange () I do get more information out of it.
...
**Answer:** Similar #1: str_to_title. The entire word is now turning into uppercase but now you're about to go to sleep. Similar: #2 str_to_replace. It is also not the whole word being uppercased, but it also is trying to look herself and look less like a common citizen. And I credit chatgpt to the last similarity.
...
**Answer:** After changing the name of the function from maryland_expenses to maryland_expenses_with_race. Afterwards the case wants to meet up with every states version.
...
**Answer:** Yes this makes sense because the code is saying all of the maryland_expenses_with_state is going to be filtered by grouping the states. Then summarize all of the grouped states and arrange them from descending greatest amount.
...
1.  Do I have to rename the data?

2.  When do I know I have to rename the data like the pre_lab did earlier?

3.  How does including the date() function change the format of the date? From m/d/y to y/d/m?
