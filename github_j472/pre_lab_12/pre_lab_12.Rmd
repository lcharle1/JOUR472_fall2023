...
**Task** Run the following code and describe the dataframe it outputs. **Answer:** There are 4 variables with a total of 999 observations. The data consists of the date, title, url, and the text of the observations.
...
**Task** Run the following code and describe what the `unique` function does to the original list of words. **Answer:** this helps us not see repeating words. But the words that popped up at the bottom are counted as unique because it is different.
...
**Task** Run the following code and describe the output of using the `unnest_tokens` function. **Answer:** Now we have a table showing all the unique words in the releases. Words that are not written properly and detected as unique.
...
**Task** Run the following code and describe what the resulting graphic shows. Is it interesting? **Answer:** We have a bar graph showing the highest to lowest used unique words.
...
**Task** Run the following code and describe the results. Is it more interesting than before? **Answer:** Extracting stop words, now we can see how many times specific words were mentioned in the speech.
...
**Task** Run the following code and describe the results. **Answer:** Now for each year, we have a list of specific words and how many times they were mentioned in a speech. This helps to see what was mentioned a lot and from there predict what the speech was about.
...
**Task** Run the following code and describe the results. **Answer:** Now we can see the most to least frequent bigrams. Reflecting on the first stop words list, we have more context on why the words we used.
...
**Task** Run the following code and describe the results. **Answer:** Same thing but for 2022, health related terms are used less and more is said about project location.
...
**Task** Run the following code and describe the results. **Answer:** For 2023, we review the same thing. Human rights takes the lead, followed by equal rights, and health care. Chesapeake Bay is always in the top 10, but COVID 19 is now off of the list.
...
**Task** Run the following code and describe the results. Do any of the sentiments seem incorrect or counter-intuitive? **Answer:** This table shows the positive or negative sentiment correlated with each word. The words are also arranged in alphabetical order.
