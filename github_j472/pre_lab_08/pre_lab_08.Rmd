...
**Task** Run the following code to add a month column to our dataframe using lubridate and count the number of calls for each month. In the space below, explain what each step of the code below is doing in English. Note the "label=TRUE" part in the mutate statement. **Answer:** First mutate a new column so you have a list of the month with the highest calls. Label = true enables this without having the months represented in numbers. Lastly, arranging them in descending order by total_calls, so we see the highest number of calls.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer:** I see a bar chart. Listing the months, in chronological order, showing the number of overdose calls they received in that month. To the left shows the count of calls.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer:** A reordered version of the overdose calls per month chart. The new order focuses on showing the least to greatest amount of overdose calls, meaning the months are not in chronological order.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer:** Now the chart is showing the bars vertically, instead of horizontally.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer:** The bars are minimized/they shrunk. The weight is the same but the size of each bar is smaller. And the background is lighter.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last version of the chart? **Answer:** Now the background is light blue, the line has changed to white, and the font has changed.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer:** There is a new function called labs(), and inside it you can write titles, and label x and y line. Now the chart has a title, More 911 Overdose Calls in Warmer Months, and the x line is called total calls, and the y line is called months. Also there is a caption saying "source: Baltimore county"
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer:** The total calls are now grouped by date. Data that is more fluctuating for the line chart.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**: A very chaotic line chart of the overdose calls.
...
A new function is included to separate the data per week, but now the x line is very chaotic too.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer:** Some space was freed up but it is still chaotic.
