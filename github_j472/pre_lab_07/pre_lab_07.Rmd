...
```{r echo=FALSE}
census_api_key("a37af987c829a192e69fdb51817d12e626ecf15d", install=TRUE, overwrite=TRUE)
```
...
**Task** Run the following codeblock to calculate the percentage change and put the state with the largest percentage growth below. **Answer:** Nevada with a 35% change
...
**Task** Run the following codeblock to find the Maryland county with the highest median household income and put the answer below. **Answer:** Howard County has the highest median household income in Maryland with \$129,549.
...
**Task** Run the following codeblock to create a chart showing the margin of error for each county. The current headline isn't very descriptive of the data. What should it be? **Answer:** Maryland County's Household Median Income.
...
Is the difference between Calvert and Montgomery significant? **Answer:** The difference is not significant because most of Montgomery inside the range of Calvert.

Is the difference between Howard and everyone else significant? **Answer:** It is because it is the farthest on the estimate and there is no one else within their range.
...
md_high_schools_with_acs |>filter(percent_of_high_school_graduates_enrolled <= 40) |> summarize(avg_income = mean(estimate), count = n())
```

**Task** What is another question you could ask of this data? **Answer:** I would ask if their an ethnic or racial component to it? Are the higher income households minorities or majorities?
