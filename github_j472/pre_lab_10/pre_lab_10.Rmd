...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **Answer:** This data set contains Maryland counties, their zip codes, shape area, shape length, and geometry.
...
**Task** Run the following code. Describe the output in the space below. **Answer:** A more detailed and granular version of the zip codes in Maryland.
...
**Task** Run the following code. Describe the output in the space below. **Answer:** Tigris has been downloaded.
...
**Task** Run the following code. Describe the output in the space below. **Answer:** Now I have a data set of counties in the United States. County level subdivision of the country, which means it includes cities, parish, and burrows.
...
**Task** Run the following code. Describe the output in the space below. **Answer**: Now we have a data set of the counties in Maryland.
...
**Task** Run the following code. Describe the output in the space below. **Answer**: Now we have a map of the counties of Maryland, but the Chesapeake Bay is gone or drawn in.
...
**Task** Run the following code. Describe the output in the space below. **Answer**: Two maps were combined here to create one detailed map. Now we have a map of the counties in Maryland with the Chesapeake Bay drawn and colored as well. Making it a little bit harder to decipher, but it is more detailed.
...
**Task** Run the following code. Describe the output in the space below. **Answer:** A table pops up showing the zip codes with foreclosures in Maryland in March 2023.
...
**Task** Run the following code. Describe the output in the space below. **Answer:** Now we have a data set with more information of the zip codes with foreclosures. More information such as: shape length, existing, shape area, etc.
...
**Task** Run the following code. Describe the output in the space below. **Answer:** We now have a map with a color scheme to show the data throughout the map.
...
**Task** Run the following code. Describe the output in the space below. Be sure to input your census api key. **Answer**: We've added other libraries to create and look through data and maps.
...
**Task** Run the following code. Describe the output in the space below. **Answer:** Now we have two new data sets showing the zip codes with foreclosure's population and the notices they received.
...
**Task** Run the following code. Describe the output in the space below. **Answer:** Now we have reshaped the data set and condensed and it has more information with the new header names.
...
**Task** Run the following code. Describe the output in the space below. **Answer:** A chart has been created using Maryland county's population data set the notices of intent to foreclosure.
...
**Task** Run the following code. Describe the output in the space below. **Answer:** A lot has been cleanred up from the md_county\_ population. County has been renamed as name. Two new columns have been created. Population was renamed estimate.
...
**Task** Run the following code. Describe the output in the space below. **Answer:** We've joined md_pop_with_foreclosures and md_county_population by county.
...
**Task** Run the following code. Describe the output in the space below. **Answer:** Left joined md_county_population and md_county_notices by county to a new data set called md_pop_with_foreclosures.
...
**Task** Run the following code. Describe the output in the space below. **Answer:** Now there is a map with the counties matched to the corresponding area with the coordinates loaded below.
...
**Task** Run the following code. Describe the output in the space below. **Answer:** Now the graph has a variety of color to further depict the different ranking counties.
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer:** The darker the county the less there are foreclosures. The brighter, the more there are foreclosures.
